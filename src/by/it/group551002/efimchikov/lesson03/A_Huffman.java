package by.it.group551002.efimchikov.lesson03;
...
public class A_Huffman {
    ...
    String encode(InputStream inputStream) throws FileNotFoundException {
        ...
        for( Character i: count.keySet()) {
            var leafnode = new LeafNode(count.get(i), i);
            priorityQueue.add(leafnode);
        }

        while(priorityQueue.size()!=1){
            var left = priorityQueue.remove();
            var right=priorityQueue.remove();
            var joint = new InternalNode(left,right);
            priorityQueue.add(joint);
        }
